...
class Subscriber(Node):
    ...
    def read_serial(self):
        ...
        self.response_serial = self.ser.read(1)     ###### .decode()

        print('Response : ' + len(self.response_serial))
        
        # Response command  
        ####self.response_command = bytes(247)  # when toggle is succesfull
        self.response_error = bytes(255)    # When it is an error 

        while True : #if self.serial_response == error_response:
            self.var = False 
    ...
    def pub_status(self):
        ...
        if self.var == False:           # if serial has error 
            status.data = "Something went wrong....."
            self.get_logger().info("Error : Something went wrong!!")
       
        else :                          # if serial has working properly     
            status.data = "Successfully lights turned on."
